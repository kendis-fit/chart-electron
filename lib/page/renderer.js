    ...
    ipcRenderer.on("init-chart", (_, chartData, chartOpts) => {
        renderChart(chartData, function() {
            if (chartOpts.saveAfterLoadingPath) {
                ...
                fs.mkdirSync(path.dirname(chartOpts.saveAfterLoadingPath), { recursive: true });
                fs.writeFileSync(chartOpts.saveAfterLoadingPath, base64, "base64");
            }
            if (chartOpts.closeAfterLoading) {
